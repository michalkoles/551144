...
class componentsGrid extends Grid
{
    ...
    protected function configure($presenter)
    {
        ...
        foreach ($gridcols as $rowdata) {
            if($rowdata->grid_visible_flag == 'Y'){
                  ...
                  if ($rowdata->db_column_name=='lst_action') {
                      $fc->setRenderer(function($row) use($presenter) {return '<a target="_blank" href="'.$presenter->link(':'.$row['module_name'].':'.$row['presenter_name'].':'.$row['lst_action']).'">'.$row['lst_action'].'</a>'; });
                      $fc->setNoescape(TRUE);
                  }
